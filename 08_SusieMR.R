...
signif_total <- readRDS('New/Pancreas/Data_MR1/MR_Result/allResults_filteredWithNames.rds')

final_moreAnalysis <- readRDS('New/Pancreas/Coloc_Toute_MR1/ColocGenesToProxy.rds')
...
for (i in 1:length(signif_total$geneName)){
  ...
  outc_RSID <- readRDS(sprintf('New/Spleen/Data_MR8/Preprocessed_Data/GCST4023_chr%s.rds', chrI))
  ...
  saveRDS(tmp_new, sprintf("New/Spleen/Plink8/tmp/tmp_Chr%s_%s.rds", chrI, prot))
  write.table(tmp_new$variant_id.x, sprintf('New/Spleen/Plink8/testChr%s_%s.txt', chrI, prot), row.names = FALSE, col.names = FALSE, quote = FALSE) #Use in Plink
}
...
for (i in 1:length(signif_total$geneName)){
  ...
  plinkLD_raw <- function(raw_files = sprintf("New/Spleen/Plink8/susieChr%s_%s.raw", chrI, prot)) {
  ...
  tmps <- plinkLD_raw(sprintf("New/Spleen/Plink8/susieChr%s_%s.raw", chrI, prot))
  ...
  tmp_new <- readRDS(sprintf("New/Spleen/Plink8/tmp/tmp_Chr%s_%s.rds", chrI, prot))
  # Merge outc_plink with tmp file that has the RSIDs
  tmp_new_susie <- merge.data.frame(outc_plink, tmp_new, by.x = 1, by.y=35)
  ...
  saveRDS(dat_susie, sprintf('New/Spleen/Plink8/susieResults/dat_susieChr%s_%s.rds', chrI, prot))
  saveRDS(ld_r, sprintf('New/Spleen/Plink8/susieResults/LD_MatrixChr%s_%s.rds', chrI, prot))
  ...
  colnames(dat_susie) <- c("rsid", "ld_eff_all", "snp", "effect_allele", "other_allele", "effect_allele.outcome", "other_allele.outcome", "beta.exposure", "beta.outcome", "eaf.exposure", "eaf.outcome", "remove", "palindromic", "ambiguous", "id.outcome","variant_id.x", "pval.outcome", "chromosome", "base_pair_location", "se.outcome","sample_size","outcome", "gene_id", "chromosome.y", "base_pair_location.y", "tss_distance", "ma_samples", "ma_count", "pval.exposure",  "se.exposure", "id.exposure", "exposure", "action", "SNP_index", "mr_keep", "samplesize.outcome", "z_expo", "z_outc")
  ...
  saveRDS(res, sprintf("New/Spleen/Plink8/susieResults/coloc_susie_Chr%s_%s.rds", chrI, prot))
  saveRDS(res$summary , sprintf("New/Spleen/Plink8/susieResults/susieResults_%s_%s.rds", chrI, prot))
...
for (i in 1:length(signif_total$geneName)){
  ...
  res <- readRDS(sprintf("New/Spleen/Plink8/susieResults/coloc_susie_Chr%s_%s.rds", chrI, prot))
...
saveRDS(susieAllResults_, "New/Spleen/Plink8/susieResults/allSignificantSusiesChrome6.rds")
