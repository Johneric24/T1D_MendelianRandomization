...
signif_total <- readRDS('New/Spleen/Data_MR10/MR_Result/allResults_filteredWithNames.rds')
signif_total <- signif_total[(signif_total$chromosome == 6 & signif_total$geneName == "PRSS16"),]
...
for (i in 1:length(signif_total$geneName)){
  ...
  outc_interest$SNP_Original <- outc_interest$SNP_ID
  
  proxied <- readRDS("Pancreas/Data_MR2/Proxy/Proxy_GWAS_MichalekEUR_Pancreas_val.rds")
  ...
  saveRDS(harm_data, sprintf('New/Spleen/Coloc_Toute_MR10/Coloc_Harm_%s_result.rds', prot))
  ...
  saveRDS(D1_expo, sprintf('New/Spleen/Coloc_Toute_MR10/Coloc_Expo_%s_result.rds', prot))
  saveRDS(D2_outc, sprintf('New/Spleen/Coloc_Toute_MR10/Coloc_Outc_%s_result.rds', prot))
...
saveRDS(tab_res, 'New/Spleen/Coloc_Toute_MR10/Genes/Coloc_result_chrome6.rds')

# Filter for genes that passed colocalization 
final_res <- tab_res[tab_res$H4 > 0.8,] 
final_moreAnalysis <- tab_res[!(tab_res$H4 > 0.8),]

saveRDS(final_res,'New/Spleen/Coloc_Toute_MR10/passingColocGenes.rds')
saveRDS(final_moreAnalysis,'New/Spleen/Coloc_Toute_MR10/ColocGenesToProxy.rds')


write.table(final_res$`Gene Name`, 'New/Spleen/Coloc_Toute_MR10/PassingGeneNames.txt', quote = FALSE, row.names = FALSE, col.names = FALSE)

final_res <- readRDS('New/Spleen/Coloc_Toute_MR10/passingColocGenes.rds')
final_moreAnalysis <- readRDS('New/Spleen/Coloc_Toute_MR10/ColocGenesToProxy.rds')
final_all <- rbind(final_res, final_moreAnalysis)
saveRDS(final_all, 'New/Spleen/Coloc_Toute_MR10/Coloc_result.rds')
